...
const CourseCard = ({ course, index = 0 }) => {
  ...
  const isInCart = cartItems.some(item => (item._id || item.id) === courseId);
    ...
    <motion.div variants={itemVariants}>
      <div className="bg-white rounded-2xl overflow-hidden group hover:shadow-xl transition-all duration-300 border border-gray-100 h-full flex flex-col">
        ...
        <div className="relative overflow-hidden">
          <img
            src={course.thumbnail || 'https://images.unsplash.com/photo-1498050108023-c5249f4df085?w=400&h=250&fit=crop'}
            alt={course.title}
        ...
        <div className="p-6 flex-1 flex flex-col">
          <div className="flex justify-between items-center mb-4 text-[#6C706F] text-[13px] font-medium">
            <div className="flex items-center">
              <Play className="w-4 h-4 mr-1.5 text-[#07A698]" /> {course.courseCurriculum?.length || 0} Modules
            </div>
            <div className="flex items-center">
              <Users className="w-4 h-4 mr-1.5 text-[#07A698]" /> {(course.students || 0).toLocaleString()} Students
            </div>
          </div>
...
<Link to={`/courses/${courseId}`}>{course.title}</Link>
          ...
          <div className="flex items-center justify-between mt-auto">
            <div className="flex items-center gap-2">
              <span className="text-[22px] font-bold text-[#162726]">${course.price || 0}</span>
            ...
            <button
              onClick={() => addToCart({ ...course, id: courseId })}
              disabled={isInCart}
